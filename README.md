...
Every figure well inside the gate, at both sizes, for every ticker. Re-run it yourself.
...
Cost, small. A parameterized read path and frontend work, no new contract required.
...
Cost, small, but deliberately not pursued yet. The rate limit and reliability budget the
vault itself depends on should not be shared with a second, less controlled surface before
