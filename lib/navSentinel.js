...
/// (e.g. "NVDAx", matched against Backed's top-level `symbol` field, confirmed against the
/// live API directly, not assumed) or an address already, resolves the X Layer address to
/// check. Works for any of Backed's 641 xStocks, not just the vault's four, since it
/// searches the same full registry resolveUnderlyingSymbol already does.
export async function resolveXStockAddress(symbolOrAddress) {
  ...
  const assets = await fetchAllBackedAssets();
  const match = assets.find((a) => a.symbol?.toLowerCase() === trimmed.toLowerCase());
  if (!match) return null;
  const xlayer = (match.deployments || []).find((d) => d.network === "XLayer");
  return xlayer?.address || xlayer?.wrapperAddressV2 || null;
}
...
export async function checkNavDeviation(xstockAddress, usdgAddress, options = {}) {
  const { symbolHint, onchainPriceUSDG, okxGetFn } = options;
