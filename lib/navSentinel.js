...
// Calibrated against real observed liquidity, not picked. Confirmed price impact across all
// five xStocks this project trades, at both $100 and $500 (README's Verified liquidity
// table), runs 0.01% to 0.21%. The previous pair here, warn 2%/block 5%, sat 25 to 500x
// above that, wide enough that only a catastrophic failure (a broken price feed, a pool that
// lost most of its liquidity) could ever reach the block threshold, not an ordinary bad
// fill. Tightened to match the already-validated closed-market pair below, which real
// observed data (see README's "Observed price tracking") already confirmed doesn't false-
// positive on legitimate movement. This still leaves roughly 5 to 10x margin over observed
// real deviations, not zero margin, so ordinary noise still passes.
export const DEFAULT_WARN_THRESHOLD_PCT = Number(process.env.NAV_WARN_THRESHOLD_PCT || 1);
export const DEFAULT_BLOCK_THRESHOLD_PCT = Number(process.env.NAV_BLOCK_THRESHOLD_PCT || 2);
...
// case this check exists to catch. Numerically identical to the open-market pair above as of
// this calibration, that's coincidence, not a merged constant, the two are independent knobs
// and can diverge again if observed conditions in either state warrant it.
