...
export async function checkNavDeviation(xstockAddress, usdgAddress, options = {}, symbolHint) {
...
export async function checkPortfolioNav(holdings, options = {}) {
  ...
  await Promise.all(
    holdings
      ...
      .map(async (h) => {
        try {
          results.set(h.address, await checkNavDeviation(h.address, USDG_ADDRESS, options, h.symbol));
        } catch (err) {
