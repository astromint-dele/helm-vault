...
function fallbackText(proposal) {
  ...
  if (proposal.action === "insufficient_funds") {
    return `I see drift from target, but I have no spendable balance to rebalance from. I need a deposit before I can propose a trade.`;
  }
...
export async function generateRebalanceProposal(vaultAddress, { driftThresholdPct = 5, navThresholdOverride } = {}) {
  ...
  if (proposal.action === "rebalance") {
    ...
    // Demo requests only: if the naturally-selected trade doesn't happen to involve a
    // blocked asset (e.g. it landed on xBTC, which has no underlying equity and can never
    // block), fall back to any blocked holding in the portfolio instead of silently doing
    // nothing. Gated on navThresholdOverride being set, so this can never widen what a real,
    // undemoed check blocks, only which asset a demo displays the same real mechanism on.
  ...
  const llmResult = await generateProposalText({
    systemPrompt:
      ...
      "are shares of total portfolio value, call those percent. If navBlockReason is set, " +
      "explain clearly that the trade was blocked for price-fairness reasons, not executed. " +
