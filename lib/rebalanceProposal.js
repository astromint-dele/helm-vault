...
export async function generateRebalanceProposal(vaultAddress, { driftThresholdPct = 5, navThresholdOverride } = {}) {
  ...
  if (proposal.action === "rebalance") {
    const fromNav = proposal.navStatus[proposal.trade.fromAddress];
    const toNav = proposal.navStatus[proposal.trade.toAddress];
    const blocked = [fromNav, toNav].find((n) => n?.status === "block");
    if (blocked) {
      ...
      proposal.navBlockReason = blocked.reason;
  ...
  const llmInput = {
    ...
    trade: proposal.trade ? { ...proposal.trade, amountHuman: Number(proposal.trade.amountHuman.toFixed(6)) } : null,
  ...
  const llmResult = await generateProposalText({
    systemPrompt:
      ...
      "digits. Do not use an em dash anywhere in your response.",
