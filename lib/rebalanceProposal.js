...
export async function generateRebalanceProposal(
  vaultAddress,
  { driftThresholdPct = 5, navThresholdOverride, timer = createTimer() } = {}
) {
  const drift = await computeDrift(vaultAddress, { timer });
  ...
  if (candidateOverweight) {
    ...
    const maxTradeSizeRaw = BigInt(
      await timer.time("rpc:maxTradeSize", () =>
        reliableRead((p) => new ethers.Contract(vaultAddress, VAULT_ABI, p).maxTradeSize(candidateOverweight.address), {
  ...
  proposal.navStatus = Object.fromEntries(
    await checkPortfolioNav(drift.holdings, { ...navThresholdOverride, timer })
  );
  ...
  // Always attached, low cost, and this is what the cold-start timing report in
  // web/app/server/vaultState.js is actually built from — real measurements, not estimates.
  proposal.timings = timer.report();
