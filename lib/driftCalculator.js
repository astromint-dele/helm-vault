...
async function getAllowedTokens(vaultAddress, timer) {
  const provider = new ethers.JsonRpcProvider(REFERENCE_RPC, 196);
  const count = Number(
    await timer.time("rpc:allowedTokenCount", () =>
      reliableRead((p) => new ethers.Contract(vaultAddress, VAULT_ABI, p).allowedTokenCount(), {
        label: "allowedTokenCount",
      })
    )
  );

  // Each allowedTokenList(i) is an independent array-index read, unrelated to the others -
  // confirmed via the timing report that running these as a for-loop cost ~2.6s of a cold
  // hit for no reason (4 tokens, ~650ms/pair, purely serialized). Fired in parallel instead.
  const indices = Array.from({ length: count }, (_, i) => i);
  const tokens = await Promise.all(
    indices.map(async (i) => {
      const address = await timer.time(`rpc:allowedTokenList:${i}`, () =>
        reliableRead((p) => new ethers.Contract(vaultAddress, VAULT_ABI, p).allowedTokenList(i), {
          label: `allowedTokenList(${i})`,
        })
      );
      const erc20 = new ethers.Contract(address, ERC20_ABI, provider);
      const [decimals, symbol, targetBps] = await timer.time(`rpc:tokenMeta:${i}`, () =>
        Promise.all([
          erc20.decimals(),
          erc20.symbol(),
      ...
      return { address, decimals: Number(decimals), symbol, targetBps: Number(targetBps) };
    })
  );
  return tokens;
}

async function getPriceInUSDG(tokenAddress, tokenDecimals, timer) {
  if (tokenAddress.toLowerCase() === USDG_ADDRESS.toLowerCase()) return 1;

  const cached = getCachedPrice(tokenAddress);
  if (cached !== null) return cached;

  const price = await timer.time(`okx:quote:${tokenAddress}`, () =>
    reliablePrice(
      async () => {
...
/// Display-only metadata (not decision-driving, so a single direct read is enough — the
/// same reasoning already applied to ERC20 symbol/decimals above): which address the vault
/// contract currently authorizes as its agent, and the current block, both read straight
/// from chain rather than hardcoded, so this stays correct if the agent is ever rotated via
/// setAgent().
export async function getVaultMeta(vaultAddress, { timer = noopTimer() } = {}) {
  const provider = new ethers.JsonRpcProvider(REFERENCE_RPC, 196);
  const vault = new ethers.Contract(vaultAddress, VAULT_ABI, provider);
  const [agentAddress, blockNumber] = await timer.time("rpc:vaultMeta", () =>
    Promise.all([vault.agent(), provider.getBlockNumber()])
  );
  return { agentAddress, blockNumber };
}
...
export async function computeDrift(vaultAddress, { timer = noopTimer() } = {}) {
  const tokens = await getAllowedTokens(vaultAddress, timer);

  const holdings = await Promise.all(
    tokens.map(async (t) => {
      const balanceRaw = await timer.time(`rpc:balance:${t.symbol}`, () => reliableBalanceOf(t.address, vaultAddress));
      const balance = Number(ethers.formatUnits(balanceRaw, t.decimals));
      const priceUSDG = await getPriceInUSDG(t.address, t.decimals, timer);
