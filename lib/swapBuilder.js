...
const MAX_BUILDER_FEE_PERCENT = 3; // OKX's own documented cap on non-Solana chains

// OKX's Builder Code fee (https://web3.okx.com/onchainos/dev-docs/trade/dex-api-addfee):
// takes a cut from either the input or output token, never both, paid to a referrer
// address we control. Off by default — both env vars must be set for it to apply, so a
// missing config can never silently misfire a fee, it just runs fee-free exactly as before.
// Deducted from the output side (toTokenReferrerWalletAddress), so the vault always spends
// exactly the input amount it decided on and simply receives a slightly smaller output,
// rather than needing to spend more than intended to net the same output.
function builderFeeParams() {
  const feePercent = process.env.OKX_BUILDER_FEE_PERCENT;
  const referrerAddress = process.env.OKX_BUILDER_REFERRER_ADDRESS;
  if (!feePercent || !referrerAddress) return {};

  const parsed = Number(feePercent);
  if (!Number.isFinite(parsed) || parsed < 0 || parsed > MAX_BUILDER_FEE_PERCENT) {
    throw new Error(
      `OKX_BUILDER_FEE_PERCENT must be between 0 and ${MAX_BUILDER_FEE_PERCENT} (OKX's own cap on non-Solana chains), got "${feePercent}".`
    );
  }
  return { toTokenReferrerWalletAddress: referrerAddress, feePercent };
}
