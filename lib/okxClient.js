...
export function createOkxClient(credentials = {}, { label = "okx" } = {}) {
  ...
  async function okxGet(path, params = {}, { timer = noopTimer() } = {}) {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      await timer.time(`okx:gateWait:${label}`, () => paceCall());
      ...
      const res = await timer.time(`okx:fetch:${label}:attempt${attempt}`, () =>
        fetch(url, {
          method: "GET",
          headers: {
